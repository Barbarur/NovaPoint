...
using NovaPointLibrary.Commands.SharePoint;
...
{
    public class SolutionPrototype : ISolution
    {
        public static string s_SolutionName = "Solution";
        public static string s_SolutionDocs = "https://github.com/Barbarur/NovaPoint/wiki/Solution-Report-ItemReport";
        ...
        public SolutionPrototype(Commands.Authentication.AppInfo appInfo, Action<LogInfo> uiAddLog, ISolutionParameters parameters)
        {
        ...
        private async Task RunScriptAsync()
        {
            ...
            SolutionProgressTracker progress = new(_main, 0);
            ...
            else
            {
                ...
                progress.IncreaseTotalCount(collSiteCollections.Count);
        ...
        private async Task ProcessSite(string siteUrl, SolutionProgressTracker progress)
        {
            ...
            try
            {
                _main.AddLogToTxt(methodName, $"Processing Site '{siteUrl}'");
        ...
        private async Task ProcessSubsites(string siteUrl, SolutionProgressTracker progress)
        {
            ...
            {
                _main.AddLogToTxt(methodName, $"Processing Subsite '{oSubsite.Title}'");
            ...
            await foreach (ListItem oItem in spoItem.Get(siteUrl, oList.Title, currentExpressions))
            {
                ...
                try
                {
                    ...
                    if (oList.BaseType == BaseType.DocumentLibrary)
                    {
                        ...
                        float itemSizeTotalMb = (float)Math.Round(fvFileSizeTotalMb.LookupId / Math.Pow(1024, 2), 2);
